...
class PIDITS(AutoRegressiveBaseModelWithCovariates):
    def __init__(
        self,
        config: dict = {},
        cell_type: str = "LSTM",
        ...
        **kwargs,
        ...
        if isinstance(target, str):  # single target
            ...
            self.distribution_projectors = nn.ModuleList(
                [nn.Linear(input_size * history_size + input_size,
    ...
    def decode_all(
        ...
        # alpha_hidden_state, beta_hidden_state = hidden_states
        # alpha_dec_output, alpha_hidden_state = self.alpha_rnn(
        #     x, alpha_hidden_state, lengths=lengths, enforce_sorted=False
        # )
        # beta_dec_output, beta_hidden_state = self.beta_rnn(
        #     x, beta_hidden_state, lengths=lengths, enforce_sorted=False
        # )
        # alpha_out = self.alpha_attention(self.alpha_projector(alpha_dec_output))
        # beta_out = self.beta_activation(self.beta_projector(beta_dec_output))
        # context = x * alpha_out * beta_out
        if context.ndim == 3:
        ...
        if idx is not None:  # Decode one step, runs on .predict()
            ...
            output = self.distribution_projectors[idx](merged_context)
            ...
            if hasattr(self, '_interpret_dict'):
                self._add_decoder_interpret(target=idx,
                                            ...
                                            W=self.distribution_projectors[idx].weight.clone().detach().numpy(),
                                            b=self.distribution_projectors[idx].bias)
            return output, context

        if isinstance(self.hparams.target, str):  # single target
            # output = self.distribution_projector(context)
            outputs = []
            for i, distribution_projector in enumerate(self.distribution_projectors):
                # Combine context with
                merged_context = torch.cat((context_sum, x[:, i, :]), dim=1)
                outputs.append(distribution_projector(merged_context))
